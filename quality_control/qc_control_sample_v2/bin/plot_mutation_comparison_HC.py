#!/usr/bin/env python3
"""
Create bar chart comparing HaplotypeCaller Joint Call vs AMP mutations (Breseq-only)
with exact and fuzzy match breakdown.

This version only counts AMP mutations detected by Breseq (BRESEQ_FREQ > 0).
"""
...
def add_value_labels(x_pos, values, bottoms, total):
    ...
    for i, (val, bottom) in enumerate(zip(values, bottoms)):
        if val > 2:  # Only show label if segment is big enough
            y_pos = bottom + val/2
            label = f'{val}'
...
plt.tight_layout()
plt.savefig('comparison/mutation_comparison_barplot_HC.png', dpi=300, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.savefig('comparison/mutation_comparison_barplot_HC.pdf', bbox_inches='tight',
            facecolor='white', edgecolor='none')

# Reset style
plt.style.use('default')
print("\nBar chart saved to:")
print("  - comparison/mutation_comparison_barplot_HC.png")
print("  - comparison/mutation_comparison_barplot_HC.pdf")

# Also create a summary text file
with open('comparison/mutation_comparison_summary_HC.txt', 'w') as f:
    f.write("MUTATION COMPARISON SUMMARY: HaplotypeCaller Joint Call vs Breseq (AMP)\n")
    f.write("=" * 70 + "\n\n")
    ...
    f.write(f"  Breseq-only: {breseq_unique} ({breseq_unique/breseq_total*100:.1f}% of Breseq)\n" if breseq_total > 0 else f"  Breseq-only: {breseq_unique}\n")
    f.write(f"  VCF-only: {vcf_unique} ({vcf_unique/vcf_total*100:.1f}% of VCF)\n\n" if vcf_total > 0 else f"  VCF-only: {vcf_unique}\n\n")

    f.write("KEY FINDING:\n")
    f.write(f"  {concordance:.1f}% concordance when allowing 50bp window for variant position\n")
    f.write(f"  Breseq detected only {breseq_total} mutations vs Joint-GATK-HC {vcf_total} variants\n")
    f.write(f"  This shows Breseq is more stringent/specific than GATK HaplotypeCaller\n")

print(f"Summary saved to: comparison/mutation_comparison_summary_HC.txt")
