...
Uses GATK-HC frequencies for first 3 categories, Breseq frequencies for Breseq-only.
"""
...
leg = ax.legend(handles=type_legend_elements, loc='lower center', fontsize=11,
                framealpha=0.8, edgecolor='black', fancybox=True,
                facecolor='white', frameon=True)
leg.get_frame().set_boxstyle('round')

# Add sample and counts annotation
stats_text = f'Sample: {sample_name}\nAllele Frequency Comparison\n(GATK-HC freq for first 3 categories)\n(Breseq freq for Breseq-only)\n\nExact: n={len(exact)}\nFuzzy: n={len(fuzzy_vcf)}\nJoint-only: n={len(vcf_only)}\nBreseq-only: n={len(breseq_only)}'
ax.text(0.02, 0.02, stats_text, transform=ax.transAxes, fontsize=10, va='bottom', ha='left',
        bbox=dict(boxstyle='round', facecolor='white', alpha=0.8, edgecolor='black'))

plt.tight_layout()
plt.savefig(f'comparison/mutation_freq_dotplot_HC_Breseq_{sample_name}.png', dpi=300, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.savefig(f'comparison/mutation_freq_dotplot_HC_Breseq_{sample_name}.pdf', bbox_inches='tight',
            facecolor='white', edgecolor='none')

plt.style.use('default')

print(f"\nFrequency dot plot saved to:")
print(f"  - comparison/mutation_freq_dotplot_HC_Breseq_{sample_name}.png")
print(f"  - comparison/mutation_freq_dotplot_HC_Breseq_{sample_name}.pdf")
