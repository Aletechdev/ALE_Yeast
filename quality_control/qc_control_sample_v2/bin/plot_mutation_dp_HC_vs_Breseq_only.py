#!/usr/bin/env python3
"""
Create dot plot showing mutation READ DEPTH (DP) for HC-only vs Breseq-only mutations.
...
# For plotting, use only positions with DP data
...
print(f"HC variants with DP: {len(vcf_data)}")
...
print(f"GATK-HC only (with DP for plotting): {len(vcf_only)}")
print(f"Breseq only: {len(breseq_only)}")

# Organize data by category and type - ONLY HC-only and Breseq-only
categories = {
    f'GATK-HC Only\n(n={len(vcf_only_all)})': (vcf_only, 'vcf'),  # Label shows total, plots only with DP
...
plt.tight_layout()
plt.savefig(f'comparison/mutation_dp_HC_vs_Breseq_only_{sample_name}.png', dpi=300, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.savefig(f'comparison/mutation_dp_HC_vs_Breseq_only_{sample_name}.pdf', bbox_inches='tight',
            facecolor='white', edgecolor='none')

plt.style.use('default')

print(f"\nHC vs Breseq-only depth plot saved to:")
print(f"  - comparison/mutation_dp_HC_vs_Breseq_only_{sample_name}.png")
print(f"  - comparison/mutation_dp_HC_vs_Breseq_only_{sample_name}.pdf")
