...
- **Profile**: `arm,docker`
- **Notes**: Nf-sarek is using some tools that stalled with Docker on Apple Silicon, e.g. multiQC, Mutect2, thus development is moved to a Azure VM
...
1. Selected nf-sarek's tools: [Prefer Mutect2 over Haptypocaller](reference_scripts/compass_artifact_wf-b8f488cc-c606-4f9a-8630-103f7c12f2bf_text_markdown.md)
1. Additional parameters: ploidy, added to the sample table as column ploidy, **TODO: pass it to FreeBayes**.
1. **TODO:** Additional NextFlow processes: VCF filter, customized for each variant calling tool, e.g. mutect alraedy subtracted control's variants, FreeBaye just show all variants from treated + control.
...
For each experiment (patient) there **has to be one normal sample** (status: 0), as I am planning to use tumor-normal mode of sarek, example below:
