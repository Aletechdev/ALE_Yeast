...
## TODO: Re-run with updated clonal AF threshold (80%)

> **June 2026**: The pipeline clonal AF hard filter was relaxed from 90% to 80%
> (see `docs/variant-calling/haplotypecaller/HARD_FILTER_HAPLOTYPECALLER_JOINT.md`).
> The results below were generated with the **old 90% threshold**. To update:
>
> 1. Change `0.90` → `0.80` and `af90` → `af80` in `02_tool_comparison.py` and `03_summary_report.py`
> 2. Re-run all four scripts in order (see Execution Order above)
> 3. Update this README's Key Conventions section accordingly
>
> This is a **nice-to-have** — the precision/recall truth set results (step 1) are unaffected,
> only the concordance tables (steps 2-3) will show slightly different clonal counts.
